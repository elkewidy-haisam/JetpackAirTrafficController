...
public class JetpackTrackingWindow extends JFrame {
    ...
    private void createRenderPanel() {
        try {
            ...
            // JOGL unavailable or initialization failed — fall back to software renderer
            System.err.println("Failed to initialize JOGL: " + e.getMessage());
            e.printStackTrace();
            JOptionPane.showMessageDialog(this,
                "JOGL unavailable — falling back to software 3D renderer.\n" + e.getMessage(),
                "JOGL Initialization Warning",
                JOptionPane.WARNING_MESSAGE);
            useJOGL = false;
            // Create a software-based MapTrackingPanel as a fallback
            renderPanel = new MapTrackingPanel(cityName, flight, allFlights, allStates, animationController);
            System.out.println("Using software Graphics2D fallback 3D rendering");
        }
    }
